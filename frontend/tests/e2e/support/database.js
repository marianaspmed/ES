...
Cypress.Commands.add('clearAllQuestionAnswers', ()=>{
  dbCommand(`
      DELETE FROM item_combination_answer_option_correspondences;
      DELETE FROM item_combination_answer_option;
...
Cypress.Commands.add('clearQuestions', ()=>{
  dbCommand(`
        DELETE from item_combination_options_correspondences ;
        DELETE from item_combination_options;
        DELETE from options ;
...
Cypress.Commands.add('clearAllQuizzes', ()=>{
  dbCommand(`
...
Cypress.Commands.add('cleanItemCombinationQuestionsByName', questionName => {
  dbCommand(`WITH toDelete AS (SELECT qt.id as question_id FROM questions qt JOIN question_details qd ON qd.question_id = qt.id and qd.question_type='item_combination' where title like '%${questionName}%')
                  , opt AS (DELETE FROM item_combination_options WHERE question_details_id IN (SELECT qd.id FROM toDelete JOIN question_details qd on qd.question_id = toDelete.question_id)) 
                  , det AS (DELETE FROM question_details WHERE question_id in (SELECT question_id FROM toDelete))
                DELETE FROM questions WHERE id IN (SELECT question_id FROM toDelete);`);
});
Cypress.Commands.add('cleanOpenEndedQuestionsByName', questionName => {
  dbCommand(`WITH toDelete AS (SELECT qt.id as question_id FROM questions qt JOIN question_details qd ON qd.question_id = qt.id and qd.question_type='open_ended' where title like '%${questionName}%')
                  , opt AS (DELETE FROM options WHERE question_details_id IN (SELECT qd.id FROM toDelete JOIN question_details qd on qd.question_id = toDelete.question_id)) 
                  , det AS (DELETE FROM question_details WHERE question_id in (SELECT question_id FROM toDelete))
                DELETE FROM questions WHERE id IN (SELECT question_id FROM toDelete);`);
});
