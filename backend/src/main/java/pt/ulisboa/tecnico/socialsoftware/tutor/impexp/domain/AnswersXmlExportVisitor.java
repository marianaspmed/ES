...
public class AnswersXmlExportVisitor implements Visitor {
    ...
    public void visitAnswerDetails(ItemCombinationAnswer answer){
        ...
        if (answer.isAnswered()) {
            ...
            answerElement.setAttribute(QUESTION_KEY, String.valueOf(answer.getQuestionAnswer().getQuestion().getKey()));
            for (ItemCombinationAnswerOption answerOption : answer.getLeftItemCombinationAnswerOptions()){
                Element correspondences = new Element("correspondences");
                answerElement.setAttribute("Sequence", String.valueOf(answerOption.getItemCombinationOption().getSequence()));
                for (Integer corr : answerOption.getCorrespondences()){
                    Element el = new Element("correspondence");
                    el.setAttribute("sequence", String.valueOf(corr));
                    correspondences.addContent(el);
                }
                answerElement.addContent(correspondences);
            }
    ...
    public void visitAnswerDetails(OpenEndedAnswer answer) {
        ...
        if (answer.isAnswered()) {
            Element answerElement = new Element("answer");
            answerElement.setAttribute(QUESTION_KEY, String.valueOf(answer.getQuestionAnswer().getQuestion().getKey()));
            answerElement.setAttribute("studentAnswer", answer.getStudentAnswer());
            this.currentElement.addContent(answerElement);
        }
    }
}
